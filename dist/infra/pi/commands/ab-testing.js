...
function summarizeVariant(variant) {
    ...
    if (variant.transcriptMaxContextPercent)
        parts.push(`ctx≤${variant.transcriptMaxContextPercent}%`);
...
async function promptVariantStrategy(ctx, currentValue) {
    const selected = await ctx.ui.select(`Suggester strategy (current: ${currentValue ?? "inherit"})`, ["(inherit)", "compact", "transcript-cache"]);
    if (!selected)
        return undefined;
    return selected === "(inherit)" ? null : selected;
}
