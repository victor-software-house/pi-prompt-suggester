...
export default function suggester(pi) {
    ...
    let cachedComposition;
    function installGhostEditor(ctx, composition) {
        if (!ctx.hasUI)
            return;
    ...
    const adapter = new PiExtensionAdapter(pi, {
        onSessionStart: async (ctx) => {
            ...
            if (composition.orchestrators.reseedRunner.isRunning())
                return;
            ...
            void composition.orchestrators.agentEnd.handle(historicalTurn, generationId, composition.runtimeRef.getAbortSignal());
        },
        onAgentEnd: async (turn, ctx) => {
            ...
            void composition.orchestrators.agentEnd.handle(turn, generationId, composition.runtimeRef.getAbortSignal());
        },
